...
const ProductsGrid = () => {
    ...
    useEffect(() => {
        if (userGroupData?.posts?.length) {
            const activePosts = userGroupData.posts.filter(el => el.active == true);
            if (activePosts.length >= 6) {
                setProducts(activePosts.slice(0,6))
                return;
            }
            if (userGroupData.posts.length >= 6) {
                const inactivePosts = userGroupData.posts.filter(el => el.active == false);
                setProducts([...activePosts, ...inactivePosts].slice(0,6));
                return;
            }
        ...
        <div
            className='h-full w-4/6 rounded-[10px] grid grid-cols-1 lg:grid-cols-3 gap-3	bg-white border border-[#E5E7EB] overflow-auto p-3'>
            {
                products.map((item) => {
                    ...
                    return (
                        <div
                            className="p-2 full max-h-full overflow-hidden rounded-lg shadow"
                            key={item._id}>
                            <div
                                className='rounded-[8px] w-full h-4/6 cursor-pointer relative'
                                onClick={() => handelModulemode(item._id)}>
                                <Image
                                    src={item
                                        .data
