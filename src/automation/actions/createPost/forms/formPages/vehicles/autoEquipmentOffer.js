...
import { autoEquipmentOffer } from "@/automation/utils/variables/formSelectors/vehicles/autoEquipmentOfferSelectors";
...
const selectorsList = (options) => [
  {
    ...
    option: () =>
      durationOfAvailabilityOfSparePartsOption(
        lebonFormOptions.autoEquipmentOffer.durationOfAvailabilityOfSpareParts.options.indexOf(
  ...
  },
  {
    ...
    option: () =>
      kindOption(
        lebonFormOptions.autoEquipmentOffer.kind.options.indexOf(options.kind) + 1
      ),
  },
];

export const autoEquipment = async (page, post) => {
  try {
    ...
    // write new price
    await writeToInput(page, post.options.price, newPrice);
