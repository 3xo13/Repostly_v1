...
import { computerAccessoriesOffer } from "@/automation/utils/variables/formSelectors/electronics/computerAccessoriesOfferSelectors";
const {
  adDescription,
  adTitle,
...
const selectorsList = (options) => [
  {
    ...
    option: () =>
      durationOfAvailabilityOfSparePartsOption(
        lebonFormOptions.computerAccessoriesOffer.durationOfAvailabilityOfSpareParts.options.indexOf(
          options.spareParts
        ) + 1
      ),
  },
  {
    ...
    option: () =>
      stateOption(
        lebonFormOptions.computerAccessoriesOffer.state.options.indexOf(
          options.state
        ) + 1
      ),
  },
...
export const computerAccessories = async (page, post) => {
  try {
